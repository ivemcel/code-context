...
export class MilvusRestfulVectorDatabase implements VectorDatabase {
    ...
    constructor(config: MilvusRestfulConfig) {
        ...
        this.baseUrl = 'http://10.142.99.29:8085';

        console.log(`🔌 Connecting to Milvus REST API at: ${this.baseUrl}`);
    }
    ...
    async dropCollection(collectionName: string): Promise<void> {
        try {
            await this.makeRequest('/codegen/collections/drop', 'POST', {
                collectionName,
                dbName: this.config.database
            });
        } catch (error) {
    ...
    async query(collectionName: string, filter: string, outputFields: string[]): Promise<Record<string, any>[]> {
        try {
            ...
            const response = await this.makeRequest('/codegen/entities/query', 'POST', queryRequest);
